...
// images
import ProgramImg from "../../../assets/display/entrepreneurship.png";
import CoreProImg from "../../../assets/display/corepro.png";
import WorkspaceImg from "../../../assets/display/workspace.png";
import ConsultingImg from "../../../assets/display/consulting.png";
...
export default function ServiceSection() {
        ...
        <section className="py-12 md:py-16">
            <div className="layout-shell mx-auto space-y-10">
                <div className={`space-y-3 ${isRTL ? "text-right" : "text-left"} md:text-inherit`}>
                    ...
                    <p
                        className={`text-sm md:text-base max-w-2xl text-core-textDark dark:text-core-textLight/90 ${
                ...
                <div className="grid gap-6 md:gap-8 md:grid-cols-2 lg:grid-cols-4">
                    {SERVICE_CARDS.map((card) => {
                        ...
                        return (
                            <article
                                key={card.id}
                                className="card-surface overflow-hidden group relative h-[420px] md:h-[460px] cursor-pointer"
                                onClick={() =>
                                    setOpenCard((prev) => (prev === card.id ? null : card.id))
                                }
                            >
                                <img
                                    src={card.imageSrc}
    ...
    ${isOpen ? "max-h-[220px]" : "max-h-[86px]"}
    md:h-[86px] md:max-h-none md:group-hover:h-[150px]
