...
export default function ProgramsPage() {
    ...
    const cmsTestimonials = useCmsItems("program_testimonials");
    ...
    return (
        <section dir={isAr ? "rtl" : "ltr"} className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-0">
            ...
            <div className="space-y-4">
                {active === "vocational" && (
                    <Vocational
                        ...
                        cmsTestimonials={cmsTestimonials
                            .filter((item) => item.program === "technical-vocational")
                            .map((item) => ({
                                name: isAr ? item.title_ar : item.title_en,
                                text: isAr ? item.body_ar : item.body_en,
                            }))}
                    />
                )}
                ...
                {active === "advanced" && (
                    <AdvancedVocational t={t} getArray={getArray} baseKey={baseKey} />
                )}
        ...
        <div className="space-y-4">
            ...
            {cmsTestimonials.length > 0 && (
                <SectionCard title={t(`${baseKey}.vocational.testimonialsTitle`)}>
                    <QuotesGrid quotes={cmsTestimonials} />
                </SectionCard>
            )}
        </div>
    );
...
function AdvancedVocational({
    ...
    return (
        <div className="space-y-4">
            <SectionCard>
                <h2 className="text-xl font-semibold text-core-textDark dark:text-core-textLight">
                    {t(`${baseKey}.advanced.title`)}
                </h2>
                <p className="mt-3 text-sm leading-7 text-core-textDark dark:text-core-textLight">
                    <span className="font-semibold text-core-textDark dark:text-core-textLight">{t(`${baseKey}.advanced.aboutTitle`)}: </span>
                    {t(`${baseKey}.advanced.about`)}
                </p>
            </SectionCard>
